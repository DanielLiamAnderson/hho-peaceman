// file: test_time_dependent_ad.cpp
// author: Daniel Anderson
//
// Tests for the HHO scheme for the time dependent
// Advection-Reaction-Diffusion equation using BDF
// time stepping.
//

#include "hho/global_interpolator.hpp"
#include "hho/integrator.hpp"
#include "hho/peaceman/time_dependent_advection_diffusion_solver.hpp"
#include "hho/post_processing.hpp"

#include "test_base.hpp"

constexpr int SPATIAL_DEGREE = 1;
...
template<size_t temporal_degree>
std::pair<std::vector<scalar_type>, std::vector<scalar_type>> test_degree(const time_dependent_adr_data& data,
  ...
  // Print header
  std::cout << std::endl << cyan << bold << "\t*** Testing degree " << temporal_degree << " ***" << reset << std::endl;

  // Create solvers and tools
  time_dependent_advection_diffusion_solver<SPATIAL_DEGREE, temporal_degree> solver(mesh);
  ...
  for (size_t j = 0; j < num_steps.size(); j++) {
    ...
    // Generate degree + 1 initial conditions (required for order degree BDF)
    std::vector<cellwise_scalar_function> initial_conditions;
    for (size_t i = 0; i < temporal_degree + 1; i++) {
      initial_conditions.push_back([=](const size_t iT, const point_type& x) {
        return data.exact_solution(data.t0 - delta_t * (temporal_degree - i), x);
      });
    }
  
    // Solve the system
    auto Xh = solver.solve(data.t0, data.t1, n_steps, data.source_term, data.boundary, data.kappa, data.beta,
    data.flux, data.mu, initial_conditions, false);
...
#define SAVE_SOLUTION_PLOTS
    ...
    for (size_t i = 0; i < Xh.size(); i++) {
      ...
      std::string filename = "./output/time_dependent_tests/" + test_name + "-" + mesh_name
        + "-degree" + std::to_string(temporal_degree) + "-" + std::to_string(i) + ".vtu";
      std::replace(filename.begin(), filename.end(), ' ', '_');
      post_processing<SPATIAL_DEGREE>(filename, solver.get_mesh(), Xh[i]);
    }
#endif  // SAVE_SOLUTION_PLOTS
  }
...
void test(const time_dependent_adr_data& data, const std::string test_name) {
  ...
  for (const auto& mesh_filename : test_meshes) {
    ...
    std::vector<std::vector<scalar_type>> errors(6, std::vector<scalar_type>(num_steps.size()));
    ...
    auto res5 = test_degree<5>(data, mesh, mesh_filename, test_name);
...
int main() {
  ...
  //              Simple diffusion -- Advection free test
  {
    ...
    test(data, "Time dependent diffusion without advection");
  }
  ...
  //              Advection test
  {
    ...
    test(data, "Time dependent advection with minor diffusion");
  }
