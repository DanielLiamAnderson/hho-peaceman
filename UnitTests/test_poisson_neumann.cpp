...
//
// Tests for the HHO scheme for Poisson with Neumann boundary conditions
//

#include "hho/common.hpp"
#include "hho/poisson_neumann_solver.hpp"
...
// Test data-set for the poisson-neumann equation
...
class tester : public generic_tester {
  ...
  test_result test(mesh_ptr& mesh, data_ptr& g_data) {
    ...
    poisson_neumann_solver<degree> solver(mesh);
...
int main() {
  ...
  // ----------------------------------------------------------------
  {
    ...
    // Neumann boundary value -- grad . n_TF
    ...
    T.add_test("Poisson Neumann zero boundary", std::shared_ptr<generic_test_data>(data));
  }
  ...
  // ----------------------------------------------------------------
  {
    ...
    data->exact_solution = [](const point_type& x) { \
      return sin(_pi * x.x()) * sin(_pi * x.y()) - 4 / (_pi * _pi);
    };
    ...
    T.add_test("Poisson Neumann nonzero boundary", std::shared_ptr<generic_test_data>(data));
  }
